...
class TSaslClientTransport(TTransportBase, CReadableTransport):
  ...
  def _recv_sasl_message(self):
    header = self._trans.read_all(5)
    status, length = struct.unpack(">BI", header)
    if length > 0:
      payload = self._trans.read_all(length)
    else:
      payload = ""
  ...
  def _read_frame(self):
    ...
    if self.encode:
      ...
      # sasl.decode()
      encoded = header + self._trans.read_all(length)
      ...
      decoded = self._trans.read_all(length)
